...
import postcssAnimations from '../dist/index';

function test(input, output, done, plOpt) {
	plOpt = plOpt || {};

	postcss(postcssAnimations(plOpt)).process(input).then(result => {
		expect(result.css.replace(/[ \n]/g, '')).to.eql(output.replace(/[ \n]/g, ''));
		done();
	});
}

describe('postcss-image-set-generator', () => {
	...
	describe('Options', () => {
		...
		it('disableCheckCssVariables: false', done => {
			...
			test(input, '' +
				...
				'@keyframes bombLeftOut{' +
					...
					'50%{opacity:1;transform-origin:-100%50%;transform:rotate(-160deg);filter:blur(0px);}' +
					'100%{opacity:0;transform-origin:-100%50%;transform:rotate(-160deg);filter:blur(20px);}' +
				'}', done, { disableCheckCssVariables: false });
		});
		...
		it('custom: [{},{},...]', done => {
			...
			test(input, '' +
				...
				'}', done, {
					custom: [
						{ customAnim:'@keyframes customAnim{0%{opacity:1;}100%{opacity:0;}}' },
						{ customAnim2: '@keyframes customAnim2{0%{opacity:0;}100%{opacity:1;}}' }
					]
				});
		});
