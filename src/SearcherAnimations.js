  ...
  appendKeyframes(css, value) {
    if (this._checkCssVariables && SearcherAnimations.isCssVariable(value)) {
      ...
      if (matchCssVars !== null) {
        ...
        matchCssVars = matchCssVars.map((el) => this.get(el.trim()));

        value = matchCssVars.reduce(
          (allValues, el) => (el ? allValues.concat(el) : allValues),
          []
        );
      }
    }
    ...
    if (Array.isArray(value)) {
      value.forEach((element) => this.appendKeyFrames(css, element));
    } else {
